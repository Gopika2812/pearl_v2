...
mongoose.connect(process.env.MONGO_URI).then(async () => {
    const p = await Product.findOne({ name: /LAZZA TRICONE PISTA/i }).lean();
    console.log('Product:', p.name, 'ID:', p._id.toString());
    const pid = p._id;
    const branchOid = p.branchId;

    const start = moment.tz('2026-04-01', 'Asia/Kolkata').startOf('day').toDate();
    const end = moment.tz('2026-04-28', 'Asia/Kolkata').endOf('day').toDate();
