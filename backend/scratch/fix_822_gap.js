...
mongoose.connect(process.env.MONGO_URI).then(async () => {
  ...
  if (oldInvoice && oldSO) {
    ...
    const invRes = await Invoice.updateOne(
      { _id: oldInvoice._id },
      { $set: { invoiceNumber: 'GEKPSI/822/26-27' } }
    );
    ...
    const soRes = await SalesOrder.updateOne(
      { _id: oldSO._id },
      { $set: { salesInvoiceId: 'GEKPSI/822/26-27' } }
    );
    console.log('SalesOrder update result:', soRes);

    // 3. Update Voucher Counter (Set it to 823 since 822 is now used)
    if (voucher) {
