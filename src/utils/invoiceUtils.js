...
export const getInvoiceHTML = (previewData, numCopies = 2, order = {}, generatedInvoice = {}) => {
    ...
    const baseTitles = isReEdited 
      ? ["RE-EDIT ORIGINAL", "RE-EDIT COPY 1"] 
      : ["ORIGINAL INVOICE", "OFFICE COPY"];
    
    const copiesToGenerate = baseTitles.slice(0, numCopies);

    copiesToGenerate.forEach(copyTitle => {
        // We use the TAX INVOICE format as the default for quick print
        html += `
          <div class="page">
            <div class="page-content">
              ...
              <div class="footer">Tax Invoice | Generated on ${new Date().toLocaleString("en-IN")}</div>
            </div>
          </div>
        `;
    });
