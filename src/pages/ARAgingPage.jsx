...
const ARAgingPage = () => {
  ...
  const fetchARaging = async () => {
    try {
      setLoading(true);
      const response = await fetch("http://localhost:5000/api/financial-reports/ar-aging");
      ...
      if (data.success) {
        ...
        setError(data.message || "Failed to fetch AR Aging");
      }
    } catch (err) {
      setError("Error fetching AR Aging: " + err.message);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleExport = () => {
    ...
    csv += "Customer Name,0-30 Days,31-60 Days,61-90 Days,90+ Days,Total\n";
    
    customers.forEach(c => {
      csv += `"${c.customerName}",${c.aged0_30 || 0},${c.aged31_60 || 0},${c.aged61_90 || 0},${c.agedOver90 || 0},${c.totalDue || 0}\n`;
    });
  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-blue-100 flex items-center justify-center pt-20 md:pt-16 md:pl-64">
        <div className="text-center">
          <FaSpinner className="text-4xl text-cyan-600 animate-spin mx-auto mb-4" />
          <p className="text-gray-600 font-semibold">Loading AR Aging Report...</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-br from-cyan-50 to-blue-100 pt-20 md:pt-16 md:pl-64 pb-10">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        {/* Header */}
        <div className="mb-8">
          <div className="flex flex-col md:flex-row md:justify-between md:items-center gap-4">
            <div>
              <h1 className="text-4xl font-bold text-gray-800 mb-2">Accounts Receivable (AR) Aging</h1>
        ...
        <div className="grid grid-cols-2 md:grid-cols-5 gap-4 mb-8">
          ...
          <div className="bg-gradient-to-br from-purple-500 to-purple-600 rounded-lg shadow-lg p-4 text-white">
            <p className="text-xs font-semibold mb-1 opacity-90">Total Due</p>
            <p className="text-2xl font-bold">
            ...
            <table className="w-full">
              ...
              <tbody>
                {customers.map((customer, idx) => (
                  <tr
                    key={idx}
                    className={`border-t ${idx % 2 === 0 ? "bg-gray-50" : "bg-white"} hover:bg-cyan-50 transition`}
                  >
                    <td className="px-6 py-4 text-sm text-gray-900 font-semibold">{customer.customerName}</td>
                    <td className="px-6 py-4 text-sm text-right text-gray-900 font-semibold">
                      {(customer.aged0_30 || 0).toLocaleString("en-IN", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}
                    </td>
                    <td className="px-6 py-4 text-sm text-right text-gray-900 font-semibold">
                      {(customer.aged31_60 || 0).toLocaleString("en-IN", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}
                    </td>
                    <td className="px-6 py-4 text-sm text-right text-gray-900 font-semibold">
                      {(customer.aged61_90 || 0).toLocaleString("en-IN", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}
                    </td>
                    <td className="px-6 py-4 text-sm text-right text-red-600 font-bold">
                      {(customer.agedOver90 || 0).toLocaleString("en-IN", {
                        minimumFractionDigits: 2,
                        maximumFractionDigits: 2
                      })}
                    </td>
              ...
              {/* Total Row */}
              <tfoot>
                <tr className="bg-cyan-600 text-white font-bold text-lg border-t-2 border-cyan-600">
