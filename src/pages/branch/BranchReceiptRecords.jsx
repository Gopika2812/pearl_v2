...
import { FaArrowLeft, FaFileAlt, FaSearch, FaTimesCircle, FaEdit, FaDownload } from "react-icons/fa";
...
export default function BranchReceiptRecords() {
  ...
  const [bankFilter, setBankFilter] = useState("ALL"); // ALL, SBI, ICICI
  ...
  const filteredReceipts = React.useMemo(() => {
    return receipts.filter(r => {
      ...
      if (searchTerm) {
        ...
        matchesTerm = id.includes(searchLower) || inv.includes(searchLower) || relatedInvs.some(rinv => rinv.includes(searchLower)) || cust.includes(searchLower) || ref.includes(searchLower);
      }
      ...
      if (activeFilter === "CASH") matchesFilter = r.paymentMethod === "CASH";
        ...
        matchesFilter = ["BANK_TRANSFER", "UPI", "CREDIT_CARD", "DEBIT_CARD"].includes(r.paymentMethod);
        if (matchesFilter && bankFilter !== "ALL") {
          const refUpper = (r.reference || "").toUpperCase();
          if (!refUpper.includes(bankFilter)) matchesFilter = false;
        }
      }
  ...
  }, [receipts, searchTerm, startDate, endDate, activeFilter, bankFilter]);
  ...
  const exportToExcel = () => {
    if (filteredReceipts.length === 0) {
      toast.warning("No receipts to export");
      return;
    }
    ...
    let fileName = `Receipt_Records_${activeFilter}`;
    if (activeFilter === "BANK" && bankFilter !== "ALL") {
      fileName += `_${bankFilter}`;
    }
    fileName += `_${new Date().toLocaleDateString('en-IN').split('/').join('-')}.xlsx`;
    ...
    <div className="min-h-screen bg-gray-50 pt-20 md:pt-4 md:pl-20">
      <div className="w-full mx-auto px-3 sm:px-6 py-4">
            ...
            <div className="md:col-span-2 flex flex-col justify-end">
              ...
              <div className="flex gap-2 items-center">
                ...
                <button
                  ...
                  className="bg-green-600 text-white px-4 py-3 rounded-xl text-xs font-bold hover:bg-green-700 transition flex-shrink-0 flex items-center gap-2"
                >
                  <FaDownload /> EXPORT
                </button>
              </div>
        ...
        <div className="flex flex-wrap gap-4 mb-8">
          ...
          <button
            onClick={() => { setActiveFilter("ALL"); setBankFilter("ALL"); }}
        ...
        {activeFilter === "BANK" && (
          <div className="flex gap-3 mb-8 justify-center">
            <button
              ...
              className={`px-6 py-2 rounded-xl text-xs font-bold transition-all border ${bankFilter === "ALL" ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"}`}
            >
              All Banks
            </button>
            <button
              onClick={() => setBankFilter("SBI")}
              className={`px-6 py-2 rounded-xl text-xs font-bold transition-all border ${bankFilter === "SBI" ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"}`}
            >
              SBI Transfers
            </button>
            <button
              onClick={() => setBankFilter("ICICI")}
              className={`px-6 py-2 rounded-xl text-xs font-bold transition-all border ${bankFilter === "ICICI" ? "bg-purple-100 text-purple-700 border-purple-200" : "bg-white text-gray-500 border-gray-200 hover:bg-gray-50"}`}
            >
              ICICI Transfers
            </button>
          </div>
