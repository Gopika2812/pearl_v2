...
async function checkStock() {
  try {
    ...
    const countWithZeroOpening = await Product.countDocuments({ openingQty: 0 });
    const countWithUndefinedOpening = await Product.countDocuments({ openingQty: { $exists: false } });

    console.log(`Total Products: ${totalCount}`);
    console.log(`Products with openingQty > 0: ${countWithOpening}`);
    console.log(`Products with openingQty = 0: ${countWithZeroOpening}`);
    console.log(`Products with openingQty missing: ${countWithUndefinedOpening}`);
